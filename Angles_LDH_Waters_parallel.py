...
The code analyze the angles between plane of AL-ZN2-ZN3 from LDH and the plane of Water (OW-HW1-H2W) 0000between the
neighbour atoms of Oxygen atom from water. The geometry is read from a Gromacs format file (*.gro)
...
def lectura_archivo(arxiu):
    ...
    m_OW=np.array([],float)
    m_HW1=np.array([],float)
    m_HW2=np.array([], float)
# Read file and filter and capture the XYZ coordinates of specific atoms
    ...
    for linea in archivo:
        ...
        if "OW" in linea:
            X,Y,Z = linea[21:45].split()[0:4]
            m_OW=np.append(m_OW,[X,Y,Z])
        if "HW1" in linea:
            X, Y, Z = linea[21:45].split()[0:4]
            m_HW1 = np.append(m_HW1, [X, Y, Z])
        if "HW2" in linea:
            X, Y, Z = linea[21:45].split()[0:4]
            m_HW2 = np.append(m_HW2, [X, Y, Z])
    archivo.close()
# Capture and manipulate the matrix of cell
    ...
    return(m_AL,m_ZN2,m_ZN3,m_OW,m_HW1,m_HW2,m_cell)
...
def angulo_molecula(j):
    # calulate distances
    distance_OWAL = np.sqrt(np.sum((np.asarray(matriz_OW[j],float)-np.asarray(matriz_nueva_AL,float))**2,axis=1))
    distance_OWZN2 = np.sqrt(np.sum((np.asarray(matriz_OW[j],float)-np.asarray(matriz_nueva_ZN2,float))**2,axis=1))
    distance_OWZN3 = np.sqrt(np.sum((np.asarray(matriz_OW[j],float)-np.asarray(matriz_nueva_ZN3,float))**2,axis=1))
    # locate the atoms of minimal distances
    indice_dOWAL=np.argmin(distance_OWAL)
    indice_dOWZN2=np.argmin(distance_OWZN2)
    indice_dOWZN3=np.argmin(distance_OWZN3)
    # Coordinates of atoms with minimal didstances
    XYZ_AL = np.asarray(matriz_nueva_AL[indice_dOWAL],float)
    XYZ_ZN2 = np.asarray(matriz_nueva_ZN2[indice_dOWZN2],float)
    XYZ_ZN3 = np.asarray(matriz_nueva_ZN3[indice_dOWZN3],float)
    XYZ_OW=np.asarray(matriz_OW[j],float)
    XYZ_HW1=np.asarray(matriz_HW1[j],float)
    XYZ_HW2=np.asarray(matriz_HW2[j],float)
    # Vectors of plane metal (AL-ZN2-ZN3) and line (Sterminal-N)
    plano_metal=np.cross((XYZ_ZN2-XYZ_AL),(XYZ_ZN3-XYZ_AL))
    plano_molecula=np.cross((XYZ_HW1-XYZ_OW),(XYZ_HW2-XYZ_OW))
...
# Extraction of geometry data form gro files of trajectory extraction
    matriz_AL,matriz_ZN2,matriz_ZN3,matriz_OW,matriz_HW1,matriz_HW2,matriz_celda=lectura_archivo('test_'+str(i)+'.gro')
    ...
    matriz_OW=np.reshape(matriz_OW,(int(len(matriz_OW)/3),3))
    matriz_HW1=np.reshape(matriz_HW1,(int(len(matriz_HW1)/3),3))
    matriz_HW2=np.reshape(matriz_HW2,(int(len(matriz_HW2)/3),3))
...
# multiprocessing the angle calculation
    if __name__ == '__main__':
        ...
        angulos.append(pool.map(angulo_molecula, range(0,len(matriz_OW))))
        pool.close()
